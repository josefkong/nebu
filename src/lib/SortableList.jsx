...
// ---------------------------------------------------------------------------
// Horizontal sortable tabs. Each tab stays clickable (a click that doesn't
// move fires onClick); a press-and-drag reorders. No drag handle/icon — the
// whole tab is the grab target, matching the "no icon on tabs" requirement.
// ---------------------------------------------------------------------------
function SortableTab({ id, children }) {
...
export function SortableTabs({ items, onReorder, renderItem }) {
  const sensors = useSensors(
    useSensor(PointerSensor, { activationConstraint: { distance: 6 } }),
    useSensor(TouchSensor, { activationConstraint: { delay: 180, tolerance: 8 } }),
    useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }),
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={ids} strategy={horizontalListSortingStrategy}>
        {items.map((it) => (
          <SortableTab key={it.key} id={it.key}>{renderItem(it)}</SortableTab>
        ))}
      </SortableContext>
    </DndContext>
  );
